...
```bash
nextflow run nf-core/bamtofastqdasl --input samplesheet.csv -profile docker
```
...
```bash
nextflow pull nf-core/bamtofastqdasl
```
...
First, go to the [nf-core/bamtofastqdasl releases page](https://github.com/nf-core/bamtofastqdasl/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/bamtofastqdasl`](https://hub.docker.com/r/nfcore/bamtofastqdasl/)
* `singularity`
